...
function select (args) {
    if (!args) throw new Error('xsql.select: Missing parameter');
    if (!(args instanceof Array)) throw new Error('xsql.select: Array argument required');
    if (!args.length) throw new Error('xsql.select: Empty array argument');
...
function from (table) {
    ...
    if ('string'!==typeof table) throw new Error('xsql.from: Non string argument');
    return 'from '+table;
}
